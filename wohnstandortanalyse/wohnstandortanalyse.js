    ...
    //WMS zbez_dropdown_view
      var zbez_dropdown_view = new ol.layer.Tile({
       ...
       if(selected_index == -1 || selected_index == 0){
        ...
        $('#map').prepend('<img id ="legend" class="legend img-rounded" src="http://student.ifip.tuwien.ac.at/geoserver/g07_2014/wms?REQUEST=GetLegendGraphic&VERSION=1.0.0&FORMAT=image/png&WIDTH=20&HEIGHT=20&STRICT=false&style=g07_zbez_dropdown_view&legend_options=fontAntiAliasing:true" />');
       }
       });
	...
	document.getElementById('p_carsharing').onclick = function(e){
	  if(this.checked==1){
		map.addLayer(lay_p_carsharing);
          if(lay_p_wohnstandortevisible=='1'){
        map.removeLayer(lay_p_wohnstandorte), map.removeLayer(lay_p_wohnstandorte_query_zbez_voronoi);
		map.addLayer(lay_p_wohnstandorte), map.addLayer(lay_p_wohnstandorte_query_zbez_voronoi);
	  }else{
	  };
	  }else{
